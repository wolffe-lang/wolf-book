...
error[E0201]: `when` requires at least two operands (one operand is a method on the sync type)
 --> ./s11.lu:7:19
  |
7 |     when (served) {
  |                   ^
...
<!-- AUDIT LEDGER (unpublished) — bs06, chapter 12. Filed to the
     ...
     - [ ] ba:diagnostic — E0201's message for the one-operand `when`
           says "one operand is a method on the sync type", and lupin's
           says "for one, call the method on the sync type". There is no
           such method: `a.lock()` answers `unsupported: `Mutex` has no
           method `lock` in this machine's std subset`. Both diagnostics
           offer a fix nobody can take. Either the single-object method
           exists (and then `[conc.when.nodeadlock]`'s argument needs
           re-checking against it) or the message should say that a
           single object is acquired by putting it in a `when` with
           whatever else the body touches.
