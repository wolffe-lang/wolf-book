...
**package** (23.0) — a directory somebody else wrote. The difference from
a module is not technical; it is that you did not read it.
...
**provenance** (9.3) — not where a pointer points, but which lineage it
inherits its permission from.
...
**task** (10.1) — a unit of concurrent work, owned by a scope. Not a
thread, not a coroutine, not a promise.
