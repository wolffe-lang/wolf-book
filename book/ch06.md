...
The tag on stderr and exit 1 — a fifth verdict alongside chapter 1's
four, and at this pin the barest one in the toolchain: no clause tag, no
span, and no trace of the calls the failure came up through. Error return
traces are a promised part of the debug profile and they do not exist
yet; when they land, this page grows the run that shows one. Until then
the honest advice is that a failure reaching `main` is a program that
...
**Step 0 — the lie, and the crash under it.** Feed the original a row
...
it says so with the code the shell can read. The text is embedded because
the pinned interpreter has no arguments and no files; the day `main`
takes an argument list, that `if` is where it arrives, and the ledger
below records the gap rather than the book pretending otherwise.
...
> `top` or `main`. That is a promise with a place to check it, §13.1, and
> the sprint that writes chapter 13 is contractually bound to the diff.
> Hold us to it. If parallelizing this program turns out to need a
> rewrite, the claim was false and the book will say so on that page.
...
<!-- AUDIT LEDGER (unpublished) — bs02, chapter 6. Filed to the
     ...
     - [ ] ba:blocker — error return traces do not exist. The sprint
           ...
           rejection, 3 trap, 4 unsupported).
     - [ ] ba:blocker (checking) — nothing about rows is checked
           ...
           whatever lands `!T` in the checker.
     - [ ] ba:blocker (evaluator) — the exemplar batch's finding 1,
     ...
     - [ ] ba:papercut — `main` cannot take arguments. reports/05
     ...
     - [ ] ba:doc-only — `errdefer` fires before an earlier `defer` on
           ...
           experiment rather than by citation.
     - [ ] ba:papercut (rig, print path) — inline HTML inside an
           ...
           should reject unknown inline HTML loudly.
     - [ ] ba:papercut (rig, found while drafting this chapter) — the
