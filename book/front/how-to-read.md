...
say so, which is the first place the language declines to guess on your
behalf. Read chapter 3's section on checked arithmetic slowly — it is
where an integer stops being a mathematical integer — and expect
...
places it is not are worth your attention: errors are values *in the
type*, so chapter 6 replaces `if err != nil` with a signature, and
there is no garbage collector, so part 2 is not optional. Chapter 11
...
**Coming from C.** Part 1 is a weekend. What is new is that the
machine's mistakes are named and stopped rather than left to you:
chapter 3's traps, chapter 2's checked slices. Chapter 20 is the one
you came for — the C membrane, and exactly how much of your existing
...
ecosystem, part 5 builds something. Exercises end the section whose
material they exercise, and every one of them has a solution in the
back except the final project's.
