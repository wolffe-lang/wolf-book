...
**Exercise 8-7** *(extension)* — Grow the ring into the full cache: an
...
Frozen data has no writers, ever, on any thread, for the rest of the
program. That is a fact about every load from it, and a fact the
...
safe to hand to another thread — no shared references to audit, no locks
to take, nothing else pointing in. Freezing gives the other half: data
with no writers is data every thread can read at once. Those two
